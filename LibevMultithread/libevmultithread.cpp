//libev多线程使用例，参考memcached使用方法。//单进程多线程
...
typedef struct conn_queue_item CQ_ITEM;  //连接条目
...
typedef struct
{
    ...
    /* accept watcher for new connect */
} DISPATCHER_THREAD;
...
static void
async_cb(EV_P_ ev_async *w, int revents)//工作线程回调函数
{
    CQ_ITEM *item;
